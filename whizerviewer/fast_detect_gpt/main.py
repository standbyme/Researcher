...
def run(args):
    ...
    with open('/zhuminjun/PAlign/LLMs-Finetuning-Safety-main/mistral/iclr_test_pred_iclr-LLama-70B-full-e1-hf_success_0.4.json') as f:
        data = json.load(f)

    data_id = list(data.keys())
    train_data_id = data_id[:400]

    eval_data_id = data_id[400:]

    texts_human = {}
    texts_model = {}
    for k,item in data.items():
        texts_human[k] = item['messages'][-1]['content']
        texts_model[k] = item['pred']['content']

    score_human = []
    score_model = []
    for i in tqdm(train_data_id):
        # evaluate text
        tokenized = scoring_tokenizer(texts_human[i], return_tensors="pt", padding=True, return_token_type_ids=False).to(args.device)
    ...
    for i in tqdm(train_data_id):
        # evaluate text
        tokenized = scoring_tokenizer(texts_model[i], return_tensors="pt", padding=True, return_token_type_ids=False).to(args.device)
        labels = tokenized.input_ids[:, 1:]
        with torch.no_grad():
            logits_score = scoring_model(**tokenized).logits[:, :-1]
            if args.reference_model_name == args.scoring_model_name:
